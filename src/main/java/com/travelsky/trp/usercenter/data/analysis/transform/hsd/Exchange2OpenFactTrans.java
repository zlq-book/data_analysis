 ...
 */
public class Exchange2OpenFactTrans {

    static final Logger logger = LoggerFactory.getLogger(Exchange2OpenFactTrans.class);
    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> exchange2OpenStream = source.flatMap((String value,
            ...
            if (!Constants.EVENT_EXCHANGE2OPEN.equals(event)) {
                return ;
            }
            logger.info("高频-客票取消换开事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                if(StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                    ...
                    logger.info("高频-客票取消换开事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }

        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("Exchange2OpenFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        exchange2OpenStream.print("exchange2OpenStream");
        // 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        exchange2OpenStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processExchange2Open(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_EXCHANGE2OPEN.equals(event)) {
            return ;
