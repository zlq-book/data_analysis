 ...
 */
public class IRRChangeFactTrans {

    static final Logger logger = LoggerFactory.getLogger(IRRChangeFactTrans.class);

    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> irrChangeStream = source.flatMap((String value,
            ...
            if (!Constants.EVENT_IRRCHANGE.equals(event)) {
                return;
            }
            logger.info("高频-客票IRR标识变更事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                // 客票IRR标识
                String irr = XpathUtils.getString(segment, "@InvoluntaryIndicator");
                ...
                // IRR标识
                tickingSegFactModel.setAkIrrflag(irr);
                ...
                if (StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                    ...
                    logger.info("高频-客票IRR标识变更事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("IRRChangeFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        irrChangeStream.print("irrChangeStream");
        // 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        irrChangeStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processIRRChange(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_IRRCHANGE.equals(event)) {
            return;
