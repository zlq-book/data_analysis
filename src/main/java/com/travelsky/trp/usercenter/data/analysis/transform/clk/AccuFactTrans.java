...
public class AccuFactTrans {

    static final Logger logger = LoggerFactory.getLogger(AccuFactTrans.class);

    public static void result(StreamTableEnvironment tEnv, String etlDate) {
        //创建Doris目标表
        tEnv.executeSql(CreateTableSql.CLK_ACCU);
        ...
        SingleOutputStreamOperator<MileAccrualFactModal> registerUdoFactModelStream = rowDataStream.map(row -> {
            ...
            String result = docName + exchNo + activityId + "ACCU";
            model.setPkId(result);
            // 数据源
            model.setMileageType("ACCU");
            // 公共部分 1-7
