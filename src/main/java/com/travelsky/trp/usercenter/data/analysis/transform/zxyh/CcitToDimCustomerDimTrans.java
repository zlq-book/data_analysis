...
public class CcitToDimCustomerDimTrans {
    static final Logger logger = LoggerFactory.getLogger(CcitToDimCustomerDimTrans.class);

    public static void result(StreamTableEnvironment tEnv, String etlDate) {
        //创建Doris目标表
        tEnv.executeSql(CreateTableSql.ZXYH_CRM_CUSTOMER_INVOICE_TITLE);
        ...
        Table dorisTable = tEnv.sqlQuery( "select \n" +
                ...
                "a.CUSTOMER_ID TID,\n" +
                "a.TAXPAYER_NAME \n" +
                "FROM T_ODS_ZXYH_CRM_CUSTOMER_INVOICE_TITLE a   "
        ...
        SingleOutputStreamOperator<CustomDimModel> customModelStream = rowDataStream.map(row -> {
            ...
            model.setInvoiceTitle(row.getField("TAXPAYER_NAME") == null ? null :
                    row.getField("TAXPAYER_NAME").toString());
