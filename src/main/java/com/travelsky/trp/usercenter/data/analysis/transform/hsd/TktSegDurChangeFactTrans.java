 ...
 */
public class TktSegDurChangeFactTrans {

    static final Logger logger = LoggerFactory.getLogger(TktSegDurChangeFactTrans.class);
    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> tktSegDurChangeStream = source.flatMap((String value, Collector<HsdProcessDataModel> out) -> {
            Document document;
            try {
            ...
            if (!Constants.EVENT_TKTSEGDURCHANGE.equals(event)) {
                return ;
            }
            logger.info("高频-客票航段有效期变更事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                // 航段有效期起始日期
                tickingSegFactModel.setTikSegstartdate(XpathUtils.getString(segment, "@NotValidBefore"));
                // 航段有效期截止日期
                tickingSegFactModel.setTikSegenddate(XpathUtils.getString(segment, "@NotValidAfter"));
                ...
                if (StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                    out.collect(hsdProcessTickingSegFactModel);
                }else{
                    logger.info("高频-客票航段有效期变更事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("TktSegDurChangeFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        tktSegDurChangeStream.print("tktSegDurChangeStream");
        // 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        tktSegDurChangeStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processTktSegDurChange(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        if (!Constants.EVENT_TKTSEGDURCHANGE.equals(event)) {
            return ;
