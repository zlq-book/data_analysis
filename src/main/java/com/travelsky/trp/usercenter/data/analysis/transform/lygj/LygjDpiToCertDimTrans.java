...
public class LygjDpiToCertDimTrans {
    //private static final Logger logger = LoggerFactory.getLogger(LygjDpiToCertDimTrans.class);
    public static void result(DataStream<JSONObject> source) {
        ...
        DataStream<CertDimModel> certDimModelDataStream = source
                .map(map -> {
                    ...
                    // 获取证件信息
                    String idCard = map.getString("CERT_NO");
                    String cardType = map.getString("ID_TYPE");
                    String tid = map.getString("TID");

                    // 检查主键是否完整
                    if (StringUtils.isBlank(idCard) || StringUtils.isBlank(cardType) || StringUtils.isBlank(tid)) {
                    ...
                    // 设置系统主键
                    model.setSystemKey(tid + cardType + idCard);

                    // 证件信息
                    model.setCertType(cardType);
        ...
        certDimModelDataStream.sinkTo(dorisSink).name("DorisSink-LygjDpiToCertDimTrans");
    }
}
