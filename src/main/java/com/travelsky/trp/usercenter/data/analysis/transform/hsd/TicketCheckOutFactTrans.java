 ...
 */
public class TicketCheckOutFactTrans {

    static final Logger logger = LoggerFactory.getLogger(TicketCheckOutFactTrans.class);
    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> ticketCheckOutStream = source.flatMap((String value, Collector<HsdProcessDataModel> out) -> {
            Document document;
            try {
            ...
            if (!Constants.EVENT_TICKETCHECKOUT.equals(event)) {
                return ;
            }
            logger.info("高频-客票值机拉下事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                if (StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                    ...
                    logger.info("高频-客票值机拉下事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("TicketCheckOutFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        ticketCheckOutStream.print("ticketCheckOutStream");
        // 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        ticketCheckOutStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processTicketCheckOut(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_TICKETCHECKOUT.equals(event)) {
            return ;
