 ...
 */
public class SplitFactTrans {

    static final Logger logger = LoggerFactory.getLogger(SplitFactTrans.class);
    ...
    public static void output(DataStream<String> source) {
        ...
        SingleOutputStreamOperator<Void> processedStream = source.process(new ProcessFunction<String, Void>() {
            ...
            public void processElement(String value, Context ctx, Collector<Void> out) throws Exception {
                ...
                processSplit(value, ctx, document);
            }
...
//        segDimDataStream.print("segDimDataStream");
    ...
    public static void processSplit(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
                /*if (!Constants.EVENT_SPLIT.equals(event)) {
                    return;
                }*/
        logger.info("高频-旅客拆分事件数据信息整合");
        // 子事件名称
        ...
        for (int a = 0; a < segmentList.getLength(); a++) {
            ...
            String segmentKey = departureDate.replace("-", "") + operatingAirline + operatingFlightNum + marketAirline + marketFlightNum + depAirport + arrivalAirport;
            ...
            if(StringUtils.isNotEmpty(departureDate) && StringUtils.isNotEmpty(operatingAirline) && StringUtils.isNotEmpty(operatingFlightNum) && StringUtils.isNotEmpty(marketAirline) && StringUtils.isNotEmpty(marketFlightNum) && StringUtils.isNotEmpty(depAirport) && StringUtils.isNotEmpty(arrivalAirport)) {
                ...
                if (StringUtils.isNotEmpty(segDimModel.getSegmentKey())) {
                    ...
                    logger.info("高频-旅客拆分事件数据信息整合[SegDimModel]数据解析异常{}", value);
                }
            }
            ...
            // PNR编号+PNR创建日期
            bookingPnrFactModel.setPkId(pnr + bookDate.replace("-", ""));
            ...
            // 是否是大客户订单
            bookingPnrFactModel.setKeyAccount(TransUtils.isKeyAccount(largeCustomerNumber));
            ...
            if (StringUtils.isNotEmpty(bookingPnrFactModel.getPkId())) {
                ...
                logger.info("高频-旅客拆分事件数据信息整合[BookingPnrFactModel]数据解析异常{}", value);
            }
            ...
            bookingSegFactModel.setPkId(pnr + bookDate.replace("-", "") + bookTime.replace(":","") + depAirport + docNumber + travellerName);
            ...
            bookingSegFactModel.setFfrf(NormalizationUtils.standardize(FieldType.FREQUENT_FLYER_NUM,
            ...
            // 是否是大客户订单
            bookingSegFactModel.setKeyAccount(TransUtils.isKeyAccount(largeCustomerNumber));
            ...
            if(StringUtils.isNotBlank(departureDate) && StringUtils.isNotBlank(bookDate)){
                LocalDate depart = LocalDate.parse(departureDate);
                LocalDate issue = LocalDate.parse(bookDate);
                // 计算两个日期之间的天数差
            ...
            // 舱等
            bookingSegFactModel.setAkCabin(TransUtils.getCabinClass(departureDate,clazz));
            ...
            if (StringUtils.isNotEmpty(bookingSegFactModel.getPkId())) {
                ...
                logger.info("高频-旅客拆分事件数据信息整合[BookingSegFactModel]数据解析异常{}", value);
            }
