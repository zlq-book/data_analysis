...
public class ExchFactTrans {

    static final Logger logger = LoggerFactory.getLogger(ExchFactTrans.class);

    public static void result(StreamTableEnvironment tEnv, String etlDate) {
        //创建Doris目标表
        tEnv.executeSql(CreateTableSql.CLK_EXCH);
        ...
        SingleOutputStreamOperator<MileRedemptionFactModal> registerUdoFactModelStream = rowDataStream.map(row -> {
            ...
            String result = docName + exchNo + activityId + "EXCH";
            model.setPkId(result);
            // 数据源
            model.setMileageType("EXCH");
            // 公共部分 1-7
