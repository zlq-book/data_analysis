...
public class CaCrmQuery {
    public static void main(String[] args) throws Exception {

            final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
            CheckpointUtils.setCheckpoint(env, "CaCrmQuery" + "LYGJ");
            env.setParallelism(1);
            final StreamTableEnvironment tEnv = StreamTableEnvironment.create(env);
            //注册数据源表
            tEnv.executeSql(CrmTableSql.getQueryTOdsDpiTmp());
            // 查询临时表
            ...
            DataStream<Tuple4<List<FfpDimModel>, List<FfpCertCheckDimModel>, List<CertDimModel>, List<UserDimModel>>> source = tEnv.toChangelogStream(table)
                ...
                public Tuple4<List<FfpDimModel>, List<FfpCertCheckDimModel>, List<CertDimModel>, List<UserDimModel>> map(Row row) throws Exception {
                    ...
                    // 常客卡级别
                    String primaryTierName = StringUtils.isNotBlank(memberInfo.getLifttimeTierENUDesc()) ? memberInfo.getLifttimeTierENUDesc() : memberInfo.getPrimaryTierName();
...
//                    + ".DPI_TMP"
//                    + " WHERE SUBSTRING(FFP, 1, 2) = 'CA'";
//            ResultSet rs = DorisUtils.getDorisResult(stmtDim, querySql);
//            try {
...
//            }
//
//            CaCrmQueryUtils.CrmQueryFun(map, "LYGJ");
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
//
