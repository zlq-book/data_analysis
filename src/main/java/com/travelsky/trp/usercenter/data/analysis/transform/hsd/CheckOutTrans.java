 ...
 */
public class CheckOutTrans {

    static final Logger logger = LoggerFactory.getLogger(CheckOutTrans.class);
    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> checkOutStream = source.flatMap((String value,
            ...
            if (!Constants.EVENT_CHECKOUT.equals(event)) {
                return ;
            }
            logger.info("高频-值机拉下事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                if(StringUtils.isNotEmpty(checkinSegFactModel.getPkId())) {
                    ...
                    logger.info("高频-值机拉下事件数据信息整合[CheckinSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("CheckInFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        checkOutStream.print("checkOutStream");
        //创建dorisSink
        // 4. 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        checkOutStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processCheckOut(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_CHECKOUT.equals(event)) {
            return ;
