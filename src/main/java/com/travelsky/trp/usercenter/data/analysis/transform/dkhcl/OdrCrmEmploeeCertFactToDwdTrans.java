...
//
public class OdrCrmEmploeeCertFactToDwdTrans {

    static final Logger logger = LoggerFactory.getLogger(OdrCrmEmploeeCertFactToDwdTrans.class);
    ...
    public static void result(StreamTableEnvironment tEnv, String etlDate) throws Exception {
        ...
        String query1 = "SELECT \n" +
                ...
                "JOIN T_ODS_ZSF_ODR_ORDER_CUSTOMER_RELATION b\n" +
                "ON a.MAIN_ORDER_NO = b.MAIN_ORDER_NO\n" +
                "WHERE a.ETL_DATE = '" + etlDate + "'\n" +
                "  AND b.ETL_DATE = '" + etlDate + "'";


        Table dorisTable1 = tEnv.sqlQuery(query1);

        DataStream<Row> rowDataStream1 = tEnv.toDataStream(dorisTable1);

        //1、预订 与 取消
        // 后续可以进行 map、filter、sink 操作
        SingleOutputStreamOperator<ZsfMultiCardBookFactModel> bookStream = rowDataStream1.map(row -> {
...
//            model.setSystemLastUpdatetime(now.toString());
        ...
        DataStream<ZsfMultiCardCancelFactModel> cancelStream = rowDataStream1.map(row -> {
...
//            model.setSystemLastUpdatetime(now.toString());
        ...
        DataStream<CertDimModel> certDimStream = rowDataStream1.map(row -> {
...
//            model.setCertExpireDate(expiryDate != null ? LocalDateTime.parse(expiryDate).toLocalDate() : null);
            model.setCertExpireDate(expiryDate != null ? LocalDate.parse(expiryDate) : null);
            //12、证件签发国
        ...
        DataStream<UserDimModel> userDimStream = rowDataStream1.map(row -> {
            ...
            if (birthdayStr != null && !birthdayStr.trim().isEmpty()) {
                ...
                model.setBirthday(LocalDate.parse(birthdayStr).toString());
            } else {
        ...
        DataStream<MobileDimModel> MobileDimModel = rowDataStream1.map(row -> {
