...
public class VariflightToSegDimTrans {
    ...
    public static void result(StreamTableEnvironment tEnv, String etlDate) throws Exception {
        ...
        DataStream<Row> keyedDataStream = rowDataStream
                .keyBy(row -> {
                    String airline = toStringSafe(row.getField("AIRLINE"));
                    String depAirport = ""; // Default
                    ...
                    String departureDate = toStringSafe(row.getField("DEPARTURE_DATE"));
                    // 使用 StringBuilder 拼接字符串
                    return new StringBuilder(depAirport).append("_").append(departureDate).toString();
                });
        ...
        SingleOutputStreamOperator<SegDimModel> mappedStream1 = rowDataStream
                ...
                .map(new RichMapFunction<Row, SegDimModel>() {
                    ...
                    public SegDimModel map(Row row) throws Exception {
                        ...
                        // 查询缓存
                        List<VariflightFlightModel> cachedFlights = flightCache.getFlights(depAirport, "DEP", departureDate);
                        if (cachedFlights == null) {
                            try {
                                // 调用 API接口 获取数据
                                List<VariflightFlightModel> apiResult = VariflightRequest.fetchAllFlights(depAirport,
                                        String.valueOf(departureDate), "DEP"
                                );
                                flightCache.putFlights(depAirport, "DEP", departureDate, apiResult);
                        ...
                        if (cachedFlights != null && !fullFlightNo.isEmpty()) {
                            StringBuilder flightKeyBuilder = new StringBuilder(fullFlightNo)
                                    .append("_").append(arrAirport);
                            matchedFlight = flightCache.getFlightByNo(flightKeyBuilder.toString(), depAirport, "DEP", departureDate);
