...
public class CbToDimUserDimTrans {
    static final Logger logger = LoggerFactory.getLogger(CbToDimUserDimTrans.class);

    public static void result(StreamTableEnvironment tEnv, String etlDate) {
        //创建Doris目标表
        tEnv.executeSql(CreateTableSql.ZXYH_CRM_BLOCKLIST);
        ...
        Table dorisTable = tEnv.sqlQuery( "SELECT \n" +
                ...
                "b.CUSTOMER_ID, \n" +
                "b.CUSTOMER_ID TID\n" +
                "FROM T_ODS_ZXYH_CRM_BLOCKLIST  a\n" +
...
//            if (row.getField("EFFECTIVE_STATUS") == null || row.getField("ENABLE") == null) {
//                return model;
//            }
//            if ("1".equals(row.getField("EFFECTIVE_STATUS").toString())
//                    && "1".equals(row.getField("ENABLE").toString())) {
//                model.setBlacklistUser(true);
//            }
//
