...
public class OtiOfiOpiToBookingPnrFactTrans {

    static final Logger logger = LoggerFactory.getLogger(OtiOfiOpiToBookingPnrFactTrans.class);
    ...
    public static void output(DataStream<Row> rowDataStream) {
        ...
        DataStream<Row> resultStream = AsyncDataStream
                .unorderedWait(
                        rowDataStream,                    // 输入流
                        new OtiOfiOpiAsync(),    // AsyncFunction
                        5000,                           // 超时：5秒
        ...
        SingleOutputStreamOperator<BookingPnrFactModel> modelStream = rowDataStream.filter(row -> {
            boolean flag = row.getField("PNR_NO") == null || row.getField("CREATE_TIME") == null;
            if (flag) {
                logger.info("入库机票预订-PNR级事实表异常，oriTable：T_ODS_LYX_ORDER_TICKET_INFO，" + row.toString());
            }
            return !flag;
