 ...
 */
public class Boarded2OpenFactTrans {

    static final Logger logger = LoggerFactory.getLogger(Boarded2OpenFactTrans.class);
    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> boarded2OpenStream = source.flatMap((String value, Collector<HsdProcessDataModel> out) -> {
            Document document;
            try {
            ...
            if (!Constants.EVENT_BOARDED2OPEN.equals(event)) {
                return;
            }
            logger.info("高频-客票登机取消事件数据信息整合");
            // 子事件名称
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                if(StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                    ...
                    logger.info("高频-客票登机取消事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("Boarded2OpenFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        boarded2OpenStream.print("boarded2OpenStream");
        // 创建 Doris Sink 并写入
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        boarded2OpenStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processBoarded2Open(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_BOARDED2OPEN.equals(event)) {
            return;
