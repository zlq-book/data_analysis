 ...
 */
public class AsrChangeFactTrans {

    static final Logger logger = LoggerFactory.getLogger(AsrChangeFactTrans.class);

    public static void output(DataStream<String> source) {

        SingleOutputStreamOperator<HsdProcessDataModel> asrChangeStream = source.flatMap((String value, Collector<HsdProcessDataModel> out) -> {
            ...
            if (!Constants.EVENT_ASRCHANGE.equals(event)) {
                return;
            }
            logger.info("高频-旅客订座系统预留座位变更事件数据信息整合");
            // 子事件名称
            ...
            // 票号
            String ticketNumber = XpathUtils.getString(passengerSegment, "TicketInfo/@TicketNumber");
            ...
            for (int a = 0; a < segmentList.getLength(); a++) {
                ...
                // 预选免费座位号 Newval
                tickingSegFactModel.setSeatNumber(newVal);
                ...
                if(StringUtils.isNotEmpty(tickingSegFactModel.getPkId())){
                    ...
                    logger.info("高频-旅客订座系统预留座位变更事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
                }
            }
        }).returns(TypeInformation.of(HsdProcessDataModel.class)).name("AsrChangeFlatMap").setParallelism(Constants.HSD_KAFKA_PARALLELISM);
        // 输出结果查看
//        asrChangeStream.print("asrChangeStream");
        //创建dorisSink
        DorisSink<HsdProcessDataModel> dorisSink = FlinkDorisUtils.creatDorisDWDSink("T_DWD_HSD_PROCESS_DATA");
        //数据写入doris
        asrChangeStream.sinkTo(dorisSink).setParallelism(Constants.HSD_KAFKA_PARALLELISM);
    }

    public static void processAsrChange(String value, ProcessFunction<String, Void>.Context ctx, Document document) {
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_ASRCHANGE.equals(event)) {
            return;
