 ...
 */
public class IDChangeFactTrans {
    static final Logger logger = LoggerFactory.getLogger(IDChangeFactTrans.class);
    public static void output(DataStream<String> source) {
        ...
        SingleOutputStreamOperator<Void> processedStream = source.process(new ProcessFunction<String, Void>() {
            ...
            public void processElement(String value, Context ctx, Collector<Void> out) throws Exception {
                ...
                processIDChange(value, ctx, document);
            }
...
//        bookingSegDataStream.print("bookingSegDataStream");
    ...
    public static void processIDChange(String value, ProcessFunction<String, Void>.Context ctx, Document document){
        // 事件名称
        String event = XpathUtils.getString(document, "/Msg/Hdr/Event");
        /*if (!Constants.EVENT_IDCHANGE.equals(event)) {
            return;
        }*/
        logger.info("高频-旅客证件信息变更事件数据信息整合");
        // 旧证件信息
        ...
        if (oldval != null && !oldval.isEmpty()) {
            ...
            if (parts.length > 1) {
                oldDocumentNumber = parts[1]; // 获取第二个部分 "M121K4291"
            }
        }
        // 子事件名称
        ...
        for (int a = 0; a < segmentList.getLength(); a++) {
            ...
            // 旧主键
            String oldPkId = pnr + bookDate + bookTime + depAirport + NormalizationUtils.standardize(FieldType.DOCUMENT_NUM,
                    SM4Utils.encrypt(oldDocumentNumber,
                            Constants.SM4_KEY)) + travellerName;
            ...
            logger.info("HSD IDChangeFactTrans bookingSegQuerySql:{}", bookingSegQuerySql);
            // 查询的数据转换
            try (Connection localConn = DorisUtils.getConnection(Constants.DORIS_JDBC_URL, Constants.DWD_USER, Constants.DWD_PWD);
                ...
                try {
                    if (bookingSegResult != null && bookingSegResult.next()) {
                        ...
                        bookingSegFactModel.setEnLastName(bookingSegResult.getString("EN_LAST_NAME"));
                        bookingSegFactModel.setEnFirstName(bookingSegResult.getString("EN_FIRST_NAME"));
                        bookingSegFactModel.setCnName(bookingSegResult.getString("CN_NAME"));
                ...
                String bookingSegDeleteSql = "DELETE FROM " + Constants.DWD_DB + ".T_DWD_BOOKING_SEG_FACT WHERE " +
                        "PK_ID = '" + oldPkId + "'";
                // 删除老数据
                logger.info("HSD IDChangeFactTrans bookingSegDeleteSql:{}", bookingSegDeleteSql);
            ...
            bookingSegFactModel.setCertNumber(docNumber);
            bookingSegFactModel.setPassengerAge(age);
            // IdMapping转换
            bookingSegFactModel.setFkPassengerUserTid(tid);
            ...
            if (StringUtils.isNotEmpty(bookingSegFactModel.getPkId())) {
                ...
                logger.info("高频-旅客证件信息变更事件数据信息整合[BookingSegFactModel]数据解析异常{}",value);
            }
            ...
            if (StringUtils.isNotEmpty(tickingSegFactModel.getPkId())) {
                ...
                logger.info("高频-旅客证件信息变更事件数据信息整合[TickingSegFactModel]数据解析异常{}",value);
            }
            ...
            if (StringUtils.isNotEmpty(tickingTicFactModel.getPkId())) {
                ...
                logger.info("高频-旅客证件信息变更事件数据信息整合[TickingTicFactModel]数据解析异常{}",value);
            }
        ...
        logger.info("HSD IDChangeFactTrans seatQuerySql:{}", seatQuerySql);
        ...
        seatTikFactModel.setCertType(NormalizationUtils.standardize(FieldType.DOCUMENT_TYPE, documentType,
        ...
        if (StringUtils.isNotEmpty(seatTikFactModel.getPkId())) {
            ...
            logger.info("高频-旅客证件信息变更事件数据信息整合[SeatTikFactModel]数据解析异常{}",value);
        }
        //}
        ...
        baggageTikFactModel.setCertType(NormalizationUtils.standardize(FieldType.DOCUMENT_TYPE, documentType,
                DataSource.HIGH_FREQUENCY_DATA));
        ...
        if (StringUtils.isNotEmpty(baggageTikFactModel.getPkId())) {
            ...
            logger.info("高频-旅客证件信息变更事件数据信息整合[BaggageTikFactModel]数据解析异常{}",value);
        }
        //}
        ...
        mealTikFactModel.setCertType(NormalizationUtils.standardize(FieldType.DOCUMENT_TYPE, documentType,
                DataSource.HIGH_FREQUENCY_DATA));
        ...
        if (StringUtils.isNotEmpty(mealTikFactModel.getPkId())) {
            ...
            logger.info("高频-旅客证件信息变更事件数据信息整合[MealTikFactModel]数据解析异常{}",value);
        }
        //}
        ...
        updgrTikFactModel.setCertType(NormalizationUtils.standardize(FieldType.DOCUMENT_TYPE, documentType,
                DataSource.HIGH_FREQUENCY_DATA));
        ...
        if (StringUtils.isNotEmpty(updgrTikFactModel.getPkId())) {
            ...
            logger.info("高频-旅客证件信息变更事件数据信息整合[UpgrTikFactModel]数据解析异常{}",value);
        }
        //}
        ...
        excessBaggageTikFactModel.setCertType(NormalizationUtils.standardize(FieldType.DOCUMENT_TYPE, documentType,
                DataSource.HIGH_FREQUENCY_DATA));
        ...
        if (StringUtils.isNotEmpty(excessBaggageTikFactModel.getPkId())) {
            ...
            logger.info("高频-旅客证件信息变更事件数据信息整合[ExcessBaggageTikFactModel]数据解析异常{}",value);
        }
        //}
    }
}
