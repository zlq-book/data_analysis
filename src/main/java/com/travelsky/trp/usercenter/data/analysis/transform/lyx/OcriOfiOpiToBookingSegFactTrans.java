...
public class OcriOfiOpiToBookingSegFactTrans {

    static final Logger logger = LoggerFactory.getLogger(OcriOfiOpiToBookingSegFactTrans.class);
    ...
    public static void output(DataStream<Row> rowDataStream) {
        ...
        SingleOutputStreamOperator<BookingSegFactModel> modelStream = rowDataStream.filter(row -> {
            ...
            if (flag) {
                logger.info("入库机票预订-航段级事实表异常，oriTable：T_ODS_LYX_ORDER_CHANGE_REFUND_INFO，" + row.toString());
            }
            return !flag;
        }).flatMap((Row row, Collector<BookingSegFactModel> out) -> {
            if ("1".equals(row.getField("ORDER_TYPE").toString()) && null != row.getField("PNR_NO")
                    && null != row.getField("DEPATURE_TIME") && null != row.getField("ORIG")
                    && null != row.getField("CERT_NUM")) {
                // 改为一次入库 没有 T-1 T+1
