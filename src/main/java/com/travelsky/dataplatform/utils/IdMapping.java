...
public class IdMapping {
    ...
    private static PreparedStatement psmtMergeTid = null;
            ...
            public String map(Tuple2<String, String> stringStringTuple2) throws Exception {
                ...
                for (int i = 0; i < tidList.size(); i++) {
                    for (int j = i + 1; j < tidList.size(); j++) {
                        ...
                        System.out.println(DateTimeUtils.getCurrentDateTime()+" 开始对比： "+ tidFirst.getTid() + ", " + tidSencond.getTid());
...
//                            List<String> values = new ArrayList<>();
                            ...
                            psmtMergeTid.setString(1,tidOld);
                            psmtMergeTid.setString(2,tidEarly);
                            psmtMergeTid.addBatch();
                            insertSize++;
        ...
        }).setParallelism(4).filter(StringUtils::isNotBlank);
        map.print();
    }
        ...
        try {
            if (null == connDim || connDim.isClosed() || null == stmtDim || stmtDim.isClosed() || null == psmtTid || psmtTid.isClosed() || null == psmtStrongid || psmtStrongid.isClosed()) {
                synchronized (lock) {
                    ...
                    if (null == psmtMergeTid || psmtMergeTid.isClosed()) {
                        System.out.println("create database statement:" + Thread.currentThread().getId() + "," + Thread.currentThread().getName());
                        String mergeTidInsertSql = "insert into " + Constants.DIM_DB + ".T_DIM_MERGE_TID (MERGED_TID,NEW_TID,CREATE_TIME) values(?,?,CURRENT_TIMESTAMP(3)) ";
                        psmtMergeTid = DorisUtils.prepareSql(connDim, mergeTidInsertSql);
                        psmtMergeTid.execute("SET group_commit = async_mode;");
                    }
                }
            }
            //自动提交关闭，改为批量提交
    ...
    public static void close(boolean... flag) {
        ...
        if (commitFlag){
            try {
                ...
                if (psmtMergeTid != null && !psmtMergeTid.isClosed()) {
                    psmtMergeTid.executeBatch();
                    connDim.commit();
                    psmtMergeTid.close();
                }
