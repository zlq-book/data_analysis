...
import com.travelsky.dataplatform.module.ods.trp.TOdsScCouponRefundDetail;
...
/**
 * SC卡券退款明细数据转储脚本（SQL模式 - 推荐）
 * 使用 StreamTableEnvironment + SQL + UDF 自动化处理加解密
 * 从SFTP下载sc_b2c_report_coupon_refund_YYYYMMDD.xlsx文件并导入到ODS数据库
 * 数据流程：SFTP → HDFS → Flink SQL (AES解密 → SM4加密) → Doris
 * 注意：TRP系统使用的是SFTP协议，不是FTP协议
 */
public class ExtractTOdsScCouponRefundDetail {

    private static final Logger logger = LoggerFactory.getLogger(ExtractTOdsScCouponRefundDetail.class);

    public static void main(String[] args) {
        try {
            logger.info("开始执行SC卡券退款明细数据转储（SQL模式，经HDFS）...");
            
            if (args.length < 1) {
                logger.error("缺少ETL日期参数，格式：yyyy-MM-dd");
                logger.error("使用示例：java -cp xxx.jar ExtractTOdsScCouponRefundDetail 2025-10-11");
                System.exit(1);
            }
            ...
            fieldModeMap.put("contactPhoneNumber", TrpFlinkTransferUtils.EncryptFieldConfig.EncryptMode.DECRYPT_AND_ENCRYPT);
            fieldModeMap.put("username", TrpFlinkTransferUtils.EncryptFieldConfig.EncryptMode.ENCRYPT_ONLY);
            ...
            // 步骤2：HDFS → Flink SQL → Doris（AES解密 → SM4加密）
            // 配置需要加解密的字段（仅联系人手机号码）
            TrpFlinkTransferUtils.processDataFromHdfsWithSQL(
                    hdfsFilePath,
                    TOdsScCouponRefundDetail.class,
                    fileDate,
                    "sc_b2c_report_coupon_refund",
                    encryptConfig
            );

            logger.info("SC卡券退款明细数据转储完成！");

        } catch (Exception e) {
            logger.error("SC卡券退款明细数据转储异常: {}", e.getMessage(), e);
        }
    }
