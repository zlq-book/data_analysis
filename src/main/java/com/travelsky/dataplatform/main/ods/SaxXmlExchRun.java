...
//        String etlDate = "2024-12-02";
        ...
        if (matcher.find()) {
            fileName = "EXCH" + matcher.group(1) + matcher.group(2) + ".xml";
            destFileName = "EXCH" + matcher.group(1) + matcher.group(2) + "done.xml";
        } else {
            return;
        }
...
//            KerberosAuthUtils.initKerberosAuth(Constants.KRB5_CONF_PATH, Constants.SYS_UCV_TEST_FOR_MAPREDUCE_PRINCIPAL, Constants.SYS_UCV_TEST_FOR_MAPREDUCE_KEYTAB_PATH);
        ...
        //checkpoint设置
        CheckpointUtils.setCheckpoint(env, "SaxXmlExchRun");
        env.setParallelism(1);
        DataStreamSource<TOdsClkExchint> fileDataStreamSource = env.addSource(new XmlFileSourceExchint(path, etlDate));
        //创建dorisSink
