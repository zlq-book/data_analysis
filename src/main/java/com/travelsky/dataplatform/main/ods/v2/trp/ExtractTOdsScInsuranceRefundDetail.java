...
/**
 * SC保险退款明细数据转储脚本（SQL模式 - 推荐）
 * 使用 StreamTableEnvironment + SQL + UDF 自动化处理加解密
 * 从SFTP下载sc_b2c_report_insurance_refund_YYYYMMDD.xlsx文件并导入到ODS数据库
 * 数据流程：SFTP → HDFS → Flink SQL (AES解密 → SM4加密) → Doris
 * 注意：TRP系统使用的是SFTP协议，不是FTP协议
 */
public class ExtractTOdsScInsuranceRefundDetail {

    private static final Logger logger = LoggerFactory.getLogger(ExtractTOdsScInsuranceRefundDetail.class);

    public static void main(String[] args) {
        try {
            logger.info("开始执行SC保险退款明细数据转储（SQL模式，经HDFS）...");
            
            if (args.length < 1) {
                logger.error("缺少ETL日期参数，格式：yyyy-MM-dd");
                logger.error("使用示例：java -cp xxx.jar ExtractTOdsScInsuranceRefundDetail 2025-10-11");
                System.exit(1);
            }
            ...
            String hdfsFilePath = TrpFlinkTransferUtils.downloadFromSftpToHdfs(
                    sftpConfig,
                    "sc_b2c_report_insurance_refund",
                    fileDate
            );
            ...
            // SC保险退款明细表中没有联系人手机号码字段，因此不需要加解密
            // 步骤2：HDFS → Flink SQL → Doris（无加解密处理）
            TrpFlinkTransferUtils.processDataFromHdfsWithSQL(
                    hdfsFilePath,
                    TOdsScInsuranceRefundDetail.class,
                    fileDate,
                    "sc_b2c_report_insurance_refund",
                    encryptConfig  // 传入加密配置
            );

            logger.info("SC保险退款明细数据转储完成！");

        } catch (Exception e) {
            logger.error("SC保险退款明细数据转储异常: {}", e.getMessage(), e);
        }
    }
