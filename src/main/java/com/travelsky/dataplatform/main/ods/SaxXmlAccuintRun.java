...
import com.travelsky.dataplatform.source.XmlFileSourceAccuint;
...
public class SaxXmlAccuintRun {
    static final Logger logger = LoggerFactory.getLogger(SaxXmlAccuintRun.class);
    public static void main(String[] args) throws Exception {
        logger.info("Accuint采集开始");
        System.out.println("Accuint采集开始");
        if (args.length < 1) {
            /*加日志：etl_date参数为空*/
           return;
        }
...
//        String etlDate = "2024-12-02";
        logger.info("Accuint采集日期:" + etlDate);
        System.out.println("Accuint采集日期:" + etlDate);
        ...
        if (matcher.find()) {
            fileName = "ACCUINT" + matcher.group(1) + matcher.group(2) + ".xml";
            destFileName = "ACCUINT" + matcher.group(1) + matcher.group(2) + "done.xml";
        } else {
            return;
        }
...
//            KerberosAuthUtils.initKerberosAuth(Constants.KRB5_CONF_PATH, Constants.SYS_UCV_TEST_FOR_MAPREDUCE_PRINCIPAL, Constants.SYS_UCV_TEST_FOR_MAPREDUCE_KEYTAB_PATH);
        ...
        //checkpoint设置
        CheckpointUtils.setCheckpoint(env, "SaxXmlAccuintRun");
        env.setParallelism(1);
        DataStream<TOdsClkAccu> fileDataStreamSource = env.addSource(new XmlFileSourceAccuint(path, etlDate));
        //创建dorisSink
