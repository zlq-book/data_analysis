...
/**
 * 国航系会员数据Excel文件转储脚本
 * 从SFTP服务器读取多个sheet页数据并导入到Doris数据库
 * 数据流程:FTP(通过SOCKS5代理) → HDFS → Flink DataStream → Doris
 * <p>
 * 支持的sheet页：
 * 1. 贵宾会员数量 → T_ODS_CLK_VIP_MEMBER_TOTAL
 * 2. 会员升级 → T_ODS_CLK_MEMBER_UPGRADE
 * 3. 会员发展 → T_ODS_CLK_MEMBER_DEVELOPMENT
 * 4. 里程累积 → T_ODS_CLK_MILEAGE_ACCUMULATION
 * 5. 里程消费 → T_ODS_CLK_MILEAGE_CONSUMPTION
 * <p>
 * 使用方式：
 * java -cp xxx ExtractClkMemberDataFromExcel <ETL日期>
 *
 * @author Generated
 * @date 2025-10-22
 */
public class ExtractClkMemberDataFromExcel {

    private static final Logger logger = LoggerFactory.getLogger(ExtractClkMemberDataFromExcel.class);

    public static void main(String[] args) {
        try {
            logger.info("开始执行国航系会员数据Excel转储任务（FTP → HDFS → Doris）...");

            // 参数校验
            if (args.length < 1) {
                logger.error("缺少ETL日期参数，格式：yyyy-MM-dd");
                logger.error("使用方式: java -cp xxx ExtractClkMemberDataFromExcel <ETL日期>");
                System.exit(1);
            }
            ...
            String hdfsFilePath = downloadFromFtpToHdfs(ftpConfig, "CA_MES", fileDate);
            ...
            Map<String, List<?>> excelData = readMemberDataFromHdfs(hdfsFilePath);
            ...
            CheckpointUtils.setCheckpoint(env, "ExtractClkMemberData-" + fileDate);

            // 步骤4：处理每个sheet页的数据并写入Doris
            logger.info("步骤4: 开始处理各sheet页数据...");

            // 4.1 处理"贵宾会员数量"
            processVipMemberTotal(env, excelData, fileDate);

            // 4.2 处理"会员升级"
            processMemberUpgrade(env, excelData, fileDate);

            // 4.3 处理"会员发展"
            processMemberDevelopment(env, excelData, fileDate);

            // 4.4 处理"里程累积"
            processMileageAccumulation(env, excelData, fileDate);

            // 4.5 处理"里程消费"
            processMileageConsumption(env, excelData, fileDate);

            // 步骤5：执行Flink作业
            logger.info("步骤5: 执行Flink作业...");
            env.execute("CLK-MemberData-SFTP-Transfer-" + fileDate);

            logger.info("国航系会员数据Excel转储任务执行完成！");

        } catch (Exception e) {
            logger.error("国航系会员数据Excel转储异常: {}", e.getMessage(), e);
        }
    }
...
//        }
    ...
    /**
     * 从HDFS读取Excel文件并解析为会员数据
     */
    private static Map<String, List<?>> readMemberDataFromHdfs(String hdfsFilePath) {
        FileSystem hdfs = null;
        ...
        try {
            ...
            // 使用EasyExcelUtils解析会员数据
            Map<String, List<?>> excelData = EasyExcelUtils.readMemberDataExcelFromStream(inputStream);
     ...
     * 处理"贵宾会员数量"数据
     */
    private static void processVipMemberTotal(StreamExecutionEnvironment env,
                                              Map<String, List<?>> excelData,
                                              String etlDate) throws Exception {
        @SuppressWarnings("unchecked")
        List<TOdsClkVipMemberTotal> dataList = (List<TOdsClkVipMemberTotal>) excelData.get("贵宾会员数量");

        if (dataList == null || dataList.isEmpty()) {
            logger.warn("Sheet[贵宾会员数量]无数据，跳过处理");
            return;
        }

        logger.info("处理Sheet[贵宾会员数量]，数据条数: {}", dataList.size());

        DataStream<TOdsClkVipMemberTotal> dataStream = env.addSource(
                new ExcelDataSource<>(dataList, etlDate),
                TypeInformation.of(TOdsClkVipMemberTotal.class)
        ).name("Excel-Source-VipMemberTotal");

        DorisSink<TOdsClkVipMemberTotal> dorisSink =
                FlinkDorisUtils.creatDorisODSSinkForFTP("T_ODS_CLK_VIP_MEMBER_TOTAL");

        dataStream.sinkTo(dorisSink).name("Doris-Sink-VipMemberTotal");

        logger.info("Sheet[贵宾会员数量]数据流已配置完成");
    }

    /**
     * 处理"会员升级"数据
     */
    private static void processMemberUpgrade(StreamExecutionEnvironment env,
                                             Map<String, List<?>> excelData,
                                             String etlDate) throws Exception {
        @SuppressWarnings("unchecked")
        List<TOdsClkMemberUpgrade> dataList = (List<TOdsClkMemberUpgrade>) excelData.get("会员升级");

        if (dataList == null || dataList.isEmpty()) {
            logger.warn("Sheet[会员升级]无数据，跳过处理");
            return;
        }

        logger.info("处理Sheet[会员升级]，数据条数: {}", dataList.size());

        DataStream<TOdsClkMemberUpgrade> dataStream = env.addSource(
                new ExcelDataSource<>(dataList, etlDate),
                TypeInformation.of(TOdsClkMemberUpgrade.class)
        ).name("Excel-Source-MemberUpgrade");

        DorisSink<TOdsClkMemberUpgrade> dorisSink =
                FlinkDorisUtils.creatDorisODSSinkForFTP("T_ODS_CLK_MEMBER_UPGRADE");

        dataStream.sinkTo(dorisSink).name("Doris-Sink-MemberUpgrade");

        logger.info("Sheet[会员升级]数据流已配置完成");
    }

    /**
     * 处理"会员发展"数据
     */
    private static void processMemberDevelopment(StreamExecutionEnvironment env,
                                                 Map<String, List<?>> excelData,
                                                 String etlDate) throws Exception {
        @SuppressWarnings("unchecked")
        List<TOdsClkMemberDevelopment> dataList = (List<TOdsClkMemberDevelopment>) excelData.get("会员发展");

        if (dataList == null || dataList.isEmpty()) {
            logger.warn("Sheet[会员发展]无数据，跳过处理");
            return;
        }

        logger.info("处理Sheet[会员发展]，数据条数: {}", dataList.size());

        DataStream<TOdsClkMemberDevelopment> dataStream = env.addSource(
                new ExcelDataSource<>(dataList, etlDate),
                TypeInformation.of(TOdsClkMemberDevelopment.class)
        ).name("Excel-Source-MemberDevelopment");

        DorisSink<TOdsClkMemberDevelopment> dorisSink =
                FlinkDorisUtils.creatDorisODSSinkForFTP("T_ODS_CLK_MEMBER_DEVELOPMENT");

        dataStream.sinkTo(dorisSink).name("Doris-Sink-MemberDevelopment");

        logger.info("Sheet[会员发展]数据流已配置完成");
    }

    /**
     * 处理"里程累积"数据
     */
    private static void processMileageAccumulation(StreamExecutionEnvironment env,
                                                   Map<String, List<?>> excelData,
                                                   String etlDate) throws Exception {
        @SuppressWarnings("unchecked")
        List<TOdsClkMileageAccumulation> dataList = (List<TOdsClkMileageAccumulation>) excelData.get("里程累积");

        if (dataList == null || dataList.isEmpty()) {
            logger.warn("Sheet[里程累积]无数据，跳过处理");
            return;
        }

        logger.info("处理Sheet[里程累积]，数据条数: {}", dataList.size());

        DataStream<TOdsClkMileageAccumulation> dataStream = env.addSource(
                new ExcelDataSource<>(dataList, etlDate),
                TypeInformation.of(TOdsClkMileageAccumulation.class)
        ).name("Excel-Source-MileageAccumulation");

        DorisSink<TOdsClkMileageAccumulation> dorisSink =
                FlinkDorisUtils.creatDorisODSSinkForFTP("T_ODS_CLK_MILEAGE_ACCUMULATION");

        dataStream.sinkTo(dorisSink).name("Doris-Sink-MileageAccumulation");

        logger.info("Sheet[里程累积]数据流已配置完成");
    }

    /**
     * 处理"里程消费"数据
     */
    private static void processMileageConsumption(StreamExecutionEnvironment env,
                                                  Map<String, List<?>> excelData,
                                                  String etlDate) throws Exception {
        @SuppressWarnings("unchecked")
        List<TOdsClkMileageConsumption> dataList = (List<TOdsClkMileageConsumption>) excelData.get("里程消费");

        if (dataList == null || dataList.isEmpty()) {
            logger.warn("Sheet[里程消费]无数据，跳过处理");
            return;
        }

        logger.info("处理Sheet[里程消费]，数据条数: {}", dataList.size());

        DataStream<TOdsClkMileageConsumption> dataStream = env.addSource(
                new ExcelDataSource<>(dataList, etlDate),
                TypeInformation.of(TOdsClkMileageConsumption.class)
        ).name("Excel-Source-MileageConsumption");

        DorisSink<TOdsClkMileageConsumption> dorisSink =
                FlinkDorisUtils.creatDorisODSSinkForFTP("T_ODS_CLK_MILEAGE_CONSUMPTION");

        dataStream.sinkTo(dorisSink).name("Doris-Sink-MileageConsumption");

        logger.info("Sheet[里程消费]数据流已配置完成");
    }
