...
import com.travelsky.dataplatform.module.ods.trp.TOdsScCouponSaleDetail;
...
/**
 * SC卡券销售明细数据转储脚本（SQL模式 - 推荐）
 * 使用 StreamTableEnvironment + SQL + UDF 自动化处理加解密
 * 从SFTP下载sc_b2c_report_coupon_sell_YYYYMMDD.xlsx文件并导入到ODS数据库
 * 数据流程：SFTP → HDFS → Flink SQL (AES解密 → SM4加密) → Doris
 * 注意：TRP系统使用的是SFTP协议，不是FTP协议
 */
public class ExtractTOdsScCouponSaleDetail {

    private static final Logger logger = LoggerFactory.getLogger(ExtractTOdsScCouponSaleDetail.class);

    public static void main(String[] args) {
        try {
            logger.info("开始执行SC卡券销售明细数据转储（SQL模式，经HDFS）...");
            ...
            TrpFlinkTransferUtils.processDataFromHdfsWithSQL(
                    hdfsFilePath,
                    TOdsScCouponSaleDetail.class,
                    fileDate,
                    "sc_b2c_report_coupon_sell",
                    encryptConfig  // 传入加密配置
            );

            logger.info("SC卡券销售明细数据转储完成（SQL模式）！");

        } catch (Exception e) {
            logger.error("SC卡券销售明细数据转储异常: {}", e.getMessage(), e);
        }
    }
