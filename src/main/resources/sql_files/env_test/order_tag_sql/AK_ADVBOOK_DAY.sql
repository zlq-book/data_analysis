-- 提前出票天数（客票级）
INSERT INTO DWD_TEST.T_DWD_TICKING_TIC_FACT (PK_ID,AK_ADVBOOK_DAY)
SELECT
    ticking.PK_ID as PK_ID,
    COALESCE(DATEDIFF(seg.FK_SEG_DATE, ticking.FK_TICKETING_DATE),0) as AK_ADVBOOK_DAY
FROM
    DWD_TEST.T_DWD_TICKING_TIC_FACT ticking
        INNER JOIN (
        SELECT
            AK_TICKET_NUMBER,
            FK_ISSUE_DATE,
...
-- 提前出票天数（航段级）
-- 成行-航段级
INSERT INTO DWD_TEST.T_DWD_DEPART_SEG_FACT (PK_ID,AK_ADVBOOK_DAY)
SELECT
...
-- 机票退票航段级事实表
INSERT INTO DWD_TEST.T_DWD_REFUND_SEG_FACT (PK_ID,AK_ADVBOOK_DAY)
SELECT
