...
-- 依赖表：DWD_TEST.T_DWD_BOOKING_SEG_FACT (机票预订), DIM_TEST.T_DIM_FFP_DIM, DIM_TEST.T_DIM_CUSTOM_DIM
...
cte_dupe_check AS (
    ...
    FROM (
        SELECT T_ID, FFRF, NULL as customer_ID FROM DIM_TEST.T_DIM_FFP_DIM
        UNION ALL
        SELECT T_ID, NULL, customer_ID FROM DIM_TEST.T_DIM_CUSTOM_DIM
    ) t GROUP BY T_ID
)
