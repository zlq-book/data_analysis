...
cte_coupon_stats AS (
    ...
    FROM DWD_TEST.T_DWD_UPGRADE_COUPON_SALES_FACT ucs
...
cte_upgrade_pref_raw AS (
    ...
    FROM (
        ...
        FROM DWD_TEST.T_DWD_DATECHANGE_SEG_FACT
        ...
        SELECT
            ...
            FK_UPGRTIK_DATE
        FROM DWD_TEST.T_DWD_UPGR_TIK_FACT
        ...
        SELECT
            ...
            FLT_DATE
        FROM DWD_TEST.T_DWD_UPGRADE_INFLIGHT_BUSINESS_FACT
...
cte_inflight_stats AS (
    ...
    FROM DWD_TEST.T_DWD_UPGRADE_INFLIGHT_BUSINESS_FACT uib
...
cte_gate_stats AS (
    ...
    FROM DWD_TEST.T_DWD_UPGR_TIK_FACT uit -- 附加服务升舱出票航段事实表
