...
WITH
    ...
    FROM
        DWD_PROD.T_DWD_DEPART_SEG_FACT
    UNION ALL
    SELECT
        FK_PASSENGER_USER_TID,
        SEAT_AMOUNT_CNY
    FROM
        DWD_PROD.T_DWD_SEAT_TIK_FACT
    WHERE
        AK_EMD_STATUS = 'F'
    UNION ALL
    SELECT
        ...
        BAGGAGE_AMOUNT_CNY
    FROM
        DWD_PROD.T_DWD_BAGGAGE_TIK_FACT
    WHERE
        AK_EMD_STATUS = 'F'
    UNION ALL
    SELECT
    ...
    FROM
        DWD_PROD.T_DWD_EXCESS_BAGGAGE_TIK_FACT
    WHERE
        AK_EMD_STATUS = 'F'
    UNION ALL
    SELECT
    ...
    SELECT
        ...
        INSURANCE_AMT
    FROM
        DWD_PROD.T_DWD_AUIS_SEG_FACT
    UNION ALL
    SELECT
        FK_PASSENGER_USER_TID,
        CAST(UPGRADE_PRICE AS DECIMAL(15, 2))
    FROM
        DWD_PROD.T_DWD_UPGRADE_INFLIGHT_BUSINESS_FACT
    WHERE
    ...
    SELECT
        ...
        COUPON_VALUE
    FROM
        DWD_PROD.T_DWD_UPGRADE_COUPON_SALES_FACT
    WHERE
    ...
    FROM
        DWD_PROD.T_DWD_TRP_COUPON_SALE_FACT
),
    ...
    FROM
        DWD_PROD.T_DWD_TICKING_SEG_FACT ts
    GROUP BY
        ts.FK_BOOKING_USER_TID
)
...
SELECT
    ...
    NOW() AS UPDATE_TIME
FROM
    DIM_PROD.T_DIM_USER_DIM target
