-- 直销用户生命周期标签
INSERT INTO DIM_PROD.T_DIM_USER_DIM (PK_ID, CUSTOMER_LIFE_CYCLE_LABEL_DIRECT_SALES)
SELECT 
    user_data.PK_ID,
    CASE 
        -- 1. 导入期：今年注册，下单数 < 1
...
LEFT JOIN (
    ...
    WHERE FK_BOOKING_USER_TID IN (SELECT PK_ID FROM DIM_PROD.T_DIM_USER_DIM WHERE IS_DIRECT_USER = 1)
