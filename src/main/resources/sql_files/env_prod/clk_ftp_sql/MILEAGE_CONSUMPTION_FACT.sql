-- 国航系里程消费明细表 - ODS到DWD层数据转换脚本
-- 数据源：常旅客-国航日报汇总表-sheet5（国航系里程消费明细表）
-- 目标表：T_DWD_CLK_MILEAGE_CONSUMPTION_FACT
...
INSERT INTO DWD_PROD.T_DWD_CLK_MILEAGE_CONSUMPTION_FACT
(
    PK_ID,
    DATE,
    AIRLINE_NAME,
    FREE_TICKET_MILEAGE,
    CABIN_UPGRADE_MILEAGE,
    NON_FLIGHT_MILEAGE,
    TOTAL_MILEAGE,
    SYSTEM_CREATETIME,
...
SELECT
    ...
    ods.FREE_TICKET AS FREE_TICKET_MILEAGE,                                          -- 免票里程
    ods.CABIN_UPGRADE AS CABIN_UPGRADE_MILEAGE,                                      -- 升舱里程
...
FROM ODS_PROD.T_ODS_CLK_MILEAGE_CONSUMPTION AS ods
LEFT JOIN DWD_PROD.T_DWD_CLK_MILEAGE_CONSUMPTION_FACT AS dwd
