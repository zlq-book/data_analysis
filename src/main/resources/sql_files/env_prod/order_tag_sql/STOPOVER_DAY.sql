-- 停留天数
INSERT INTO DWD_PROD.T_DWD_TICKING_SEG_FACT (PK_ID,STOPOVER_DAY)
SELECT
    PK_ID,
    CASE
...
FROM (
         ...
         FROM
             DWD_PROD.T_DWD_TICKING_SEG_FACT t
                 INNER JOIN
             DIM_PROD.T_DIM_SEG_DIM s
             ON
...
-- （成行航段级）
INSERT INTO DWD_PROD.T_DWD_DEPART_SEG_FACT(PK_ID,STOPOVER_DAY)
SELECT
    flight_seg.PK_ID,
    ticket_seg.STOPOVER_DAY
FROM
    DWD_PROD.T_DWD_DEPART_SEG_FACT flight_seg
        INNER JOIN
    DWD_PROD.T_DWD_TICKING_SEG_FACT ticket_seg
    ON
